...
class ViSADataset2(BaseDataset):
    CATEGORIES = [
        'candle', 'capsules', 'cashew', 'chewinggum', 'fryum',
        'macaroni1', 'macaroni2', 'pcb1', 'pcb2', 'pcb3',
        'pcb4', 'pipe_fryum'
    ...
    def __init__(self, root_dir, category, split, transform=None, mask_transform=None,
        ...
        self._load_normal_samples()
        self._load_anomaly_samples()

        random.seed(seed)
        random.shuffle(self.normal_samples)
        self.split_idx = int(len(self.normal_samples) * (1 - self.test_ratio))

        super().__init__(root_dir, category, split, transform, mask_transform)

    def _load_normal_samples(self):
        normal_dir = os.path.join(self.category_dir, "Data", "Images", "Normal")
        for image_path in sorted(glob(os.path.join(normal_dir, "*.JPG"))):
    ...
    def _load_anomaly_samples(self):
        anomaly_dir = os.path.join(self.category_dir, "Data", "Images", "Anomaly")
        mask_dir = os.path.join(self.category_dir, "Data", "Masks", "Anomaly")

        for image_path in sorted(glob(os.path.join(anomaly_dir, "*.JPG"))):
            image_name = os.path.basename(image_path)
    ...
    def _load_train_samples(self):
        self.samples = self.normal_samples[:self.split_idx]

    def _load_test_samples(self):
        self.samples = self.normal_samples[self.split_idx:] + self.anomaly_samples
