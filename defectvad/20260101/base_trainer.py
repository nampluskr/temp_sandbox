...
class BaseTrainer(ABC):
    ...
    def configure_early_stoppers(self):
        self.train_early_stopper = None
        self.valid_early_stopper = EarlyStopper(patience=5, min_delta=1e-3, monitor="img_auroc")
    ...
    def on_validation_epoch_end(self, outputs):
        valid_info = ", ".join([f"{k[4:]}:{v:.3f}" for k, v in outputs.items()])
    ...
    def validation_loop(self, dataloader):
        if self.evaluator is not None:
            return self.evaluator(dataloader, pixel_level=False)
        return {}
