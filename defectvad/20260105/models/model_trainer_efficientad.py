...
class EfficientAdTrainer(BaseTrainer):
    def __init__(self, model, model_size="small"):
        if not isinstance(model, EfficientAd):
            raise TypeError(f"Unexpected  model: {type(model).__name__}")

        super().__init__(model, loss_fn=None)

        self.model_size = model_size
        self.backbone_dir = Path(os.environ["BACKBONE_DIR"])
        self.imagenet_dir = Path(os.environ["DATASET_DIR"]) / "imagenette2"
